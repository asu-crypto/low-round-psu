...
case "$1" in
    ...
    on)
        ...
        if [ ! -z "$3" ] ; then
            ...
            tc class add dev lo parent 1:1 classid 1:11 htb rate "$3"Mbit
            tc qdisc add dev lo parent 1:11 handle 10: netem delay "$2"ms limit 100000
        else
            tc qdisc add dev lo handle 1: root netem delay "$2"ms limit 100000
        fi
        ;;
    off)
