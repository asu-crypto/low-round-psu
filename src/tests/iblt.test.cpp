...
TEST_CASE("IBLT list algorithm correctly decodes all encoded elements (threshold = 2^20, n = 2^20, mult_fact = 1.5)", "[list][t=2^20][n=2^20]") {
...
TEST_CASE("IBLT cache_sensitive_list algorithm correctly decodes all encoded elements (threshold = 2^20, n = 2^10, mult_fact = 1.5)", "[cached_list][t=2^20][n=2^10]") {
    ...
    iblt::cache_sensitive_iblt_list(t, num_elements_to_insert, retrieved_vals, retrieved_counts, num_retrieved_elements);
    ...
    for (size_t i = 0; i < num_elements_to_insert; i++) {
        REQUIRE(actual_vals[i] == expected_vals[i]);
    }

}

TEST_CASE("IBLT cache_sensitive_list algorithm correctly decodes all encoded elements (threshold = 2^20, n = 2^20, mult_fact = 1.5)", "[cached_list][t=2^20][n=2^20]") {
...
TEST_CASE("IBLT queued_list algorithm correctly decodes all encoded elements (threshold = 2^20, n = 2^10, mult_fact = 1.5)", "[queued_list][t=2^20][n=2^10]") {
    ...
    size_t num_elements_to_insert = 1 << 10; // 1 million
...
TEST_CASE("IBLT queued_list algorithm correctly decodes all encoded elements (threshold = 2^20, n = 2^20, mult_fact = 1.5)", "[queued_list][t=2^20][n=2^20]") {
