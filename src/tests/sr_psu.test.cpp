...
TEST_CASE("sr_psu with n=5 input set sizes", "[sr_psu][n=5]") {
    ...
    auto p0 = sr_psu::receive(receiver_input_set, pk, sk_share1, receiver_priv_prng, socks[0], union_out, 1);
    auto p1 = sr_psu::send(sender_input_set, pk, sk_share0, sender_priv_prng, socks[1], 1);
...
TEST_CASE("sr_psu with n=128 input set sizes", "[sr_psu][n=2^7][t=16]") {
    ...
    auto p0 = sr_psu::receive(receiver_input_set, pk, sk_share1, receiver_priv_prng, socks[0], union_out, num_threads_for_parallel_ops);
    auto p1 = sr_psu::send(sender_input_set, pk, sk_share0, sender_priv_prng, socks[1], num_threads_for_parallel_ops);
...
TEST_CASE("sr_psu with n=2^14 input set sizes", "[sr_psu][n=2^14][t=16]") {
    ...
    REQUIRE(union_out.size() == expected_union.size());
    for (size_t i = 0; i < union_out.size(); ++i) {
