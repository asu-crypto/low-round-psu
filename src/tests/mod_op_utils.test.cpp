...
TEST_CASE("batch_minv_mod_mp correctly inverts 2^20 random elements mod (2^61-1)^2", "[batch][mod_inv][int128][spp]") {
    PRNG prng(osuCrypto::toBlock(15390177776208555531ULL, 11099548744950833705ULL));
...
TEST_CASE("mul_mod_spp_c correctly multiplies two semi-random elements mod (2^61-1)^2", "[mod_mul][int128][spp]") {
    PRNG prng(osuCrypto::toBlock(15390177776208555531ULL, 11099548744950833705ULL));

    size_t num_test_trials = 1 << 20;
    ...
    for (size_t i = 0; i < num_test_trials; i++) {
        unsigned __int128 a = pair_vec[0];
        unsigned __int128 b = pair_vec[1];

        unsigned __int128 res = mul_mod_spp_c(a, b);

        mpz_class mpz_a, mpz_b;
        mod_op_utils::load_int128_as_mpz(mpz_a, a);
        mod_op_utils::load_int128_as_mpz(mpz_b, b);

        mpz_class expected_res_mpz;
        mpz_mul(expected_res_mpz.get_mpz_t(), mpz_a.get_mpz_t(), mpz_b.get_mpz_t());
        mpz_mod(expected_res_mpz.get_mpz_t(), expected_res_mpz.get_mpz_t(), mod_op_utils::mpz_mod_spp.get_mpz_t());

        unsigned __int128 expected_res;
        mod_op_utils::store_mpz_as_int128(expected_res, expected_res_mpz);
